...
clone :: T.Text -> IO ()
clone repoName = Shelly.shelly $
    Shelly.run_ "git" ["clone", repoName]

checkout :: T.Text -> IO ()
checkout branchName = Shelly.shelly $
    Shelly.run_ "git" ["checkout", branchName]

merge :: T.Text -> IO ()
merge branchName = Shelly.shelly $
    Shelly.run_ "git" ["merge", branchName]

mergeNoFF :: T.Text -> IO ()
mergeNoFF branchName = Shelly.shelly $
...
forcePush :: IO ()
forcePush = Shelly.shelly $
    Shelly.run_ "git" ["push", "-f"]

rebase :: T.Text -> IO ()
rebase branchName = Shelly.shelly $
